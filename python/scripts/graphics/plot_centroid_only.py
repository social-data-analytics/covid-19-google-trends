...
for index, header in enumerate(headers):
    ...
    marker = _MARKER_LUK[int(index/10)]
    ...
    current_ax.scatter(centroid_theta_radian, centroid_r, marker=marker, 
                       label=labels[header].replace("\n", "").replace("(", "\n("), 
                       zorder=99)
...
current_ax.set_rmax(0.175)
current_ax.set_rticks([0.05, 0.10, 0.15,])
...
plt.savefig(f'./graphs/20231123-RescaledPolarProjection_CentroidOnly.pdf')
# plt.savefig(f'./graphs/20231123-RescaledPolarProjection_CentroidOnly.png')
