...
reference_df = pd.read_pickle("D:/OneDrive - HKUST/LongCOVIDCode/data/msv/.g.11qm6vy88k.pkl").query(f'date >= \'{START_DATE}\'').reset_index()
...
# with PdfPages('./graphs/20240423-LongCV-MSVs.1st.pdf') as pdf:
    for index, tag in enumerate(keyword_query_results[1:]):
        ...
        msv_df = msv_df.query(f'date >= \'{START_DATE}\'').reset_index()
        ...
        ax1.set_xlim(left=date.fromisoformat('2022-06-10'), right=date.fromisoformat('2023-02-01'))
        plt.tight_layout()

        # pdf.savefig()  # saves the current figure into a pdf page
        plt.savefig('d://OneDrive - HKUST//LongCOVIDCode//graphs//MSVPair-2ndWave-after//' + keyword + ".png")
        plt.savefig('d://OneDrive - HKUST//LongCOVIDCode//graphs//MSVPair-2ndWave-after//' + keyword + ".pdf")
        plt.close()
